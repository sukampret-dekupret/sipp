...
Partial Class admin_DaftarPermohonanSatker
    Inherits System.Web.UI.Page

    Public xPROSES As String
    Public xSIAP As String
    Public xKONFIRMASI As String
    Public xSELESAI As String

    Private Sub Admin_DaftarPermohonanSatker_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            xFungsi.delData("TBL_ORDER_DETAIL", "WHERE KDUNKER is null")


            ''Label1.Text = Session("KDUNKER").ToString & "," & Session("KDUNKERATAS")

        End If

        Try
            xPROSES = xFungsi.countData("TBL_ORDER", "KDORDER", "WHERE dbo.GetStatusORDER(KDORDER)=1").ToString
            xSIAP = xFungsi.countData("TBL_ORDER", "KDORDER", "WHERE dbo.GetStatusORDER(KDORDER)=3").ToString
            xKONFIRMASI = xFungsi.countData("TBL_ORDER", "KDORDER", "WHERE dbo.GetStatusORDER(KDORDER)=4").ToString
            xSELESAI = xFungsi.countData("TBL_ORDER", "KDORDER", "WHERE dbo.GetStatusORDER(KDORDER)=5").ToString
    ...
    '    If (e.DataColumn.FieldName.Equals("STATUS")) = 0 Then
    '        e.Cell.BackColor = System.Drawing.Color.Yellow
    '    ElseIf (e.DataColumn.FieldName.Equals("STATUS")) = 1 Then
    '        e.Cell.BackColor = System.Drawing.Color.LimeGreen
    '    ElseIf (e.DataColumn.FieldName.Equals("STATUS")) = 2 Then
    ...
    Protected Sub gridDetail_HtmlRowPrepared(sender As Object, e As ASPxGridViewTableRowEventArgs)
        ...
        ElseIf (e.GetValue("STOK2")) > (e.GetValue("STOK1")) Then
    ...
    Protected Sub btnBatal_Click(sender As Object, e As ImageClickEventArgs)
        ...
        Dim xFungsi As New FungsiData
        Dim confirmValue As String = Request.Form("confirm_value")
        'Select Case [IDETAILORDER], dbo.GetNAMABARANG(KODE_BARANG) As NAMA_BARANG, [MOHON], [PENUHI], 
        'dbo.getJMLSERTERORDERperKDBRGperUNKER(KODE_BARANG, @KDUNKER, GETDATE(), 0, 1) - dbo.getJMLMOHONperKDBRGperUNKER(KODE_BARANG, @KDUNKER, GETDATE()) As STOK2,
        '(dbo.getJMLSERTERSPKperKDBRGperUNKER(KODE_BARANG , @KDUNKER1 , getdate()) - dbo.getJMLSERTERORDERperKDBRGperUNKER(KODE_BARANG , @KDUNKER1 , getdate() , 1,0))  As STOK1,
        '[KDUNKER]  From [TBL_ORDER_DETAIL] Where KDORDER =@KDORDER
        If confirmValue = "Yes" Then
            Dim xMOHON As Integer = xFungsi.sumData("TBL_ORDER_DETAIL", "PENUHI", "KDORDER='" & xPK & "'")
            Dim xSTOK As Integer = xFungsi.sumData("TBL_ORDER_DETAIL", "(dbo.getJMLSERTERSPKperKDBRGperUNKER(KODE_BARANG , '" & Session("KDUNKER") & "' , getdate()) - dbo.getJMLSERTERORDERperKDBRGperUNKER(KODE_BARANG , '" & Session("KDUNKERATAS") & "' , getdate() , 1,0))", "KDORDER='" & xPK & "'")
            If xMOHON > xSTOK Then
                ClientScript.RegisterStartupScript(Me.[GetType](), "alert", "alert('Stok tidak ada')", True)
            End If
    ...
    Protected Sub btnProses_Click(sender As Object, e As EventArgs)
        ...
        If confirmValue = "Yes" Then
            ...
            Else
                ...
                Finally
                    grid.DataBind()
                    ClientScript.RegisterStartupScript(Me.[GetType](), "alert", "alert('Permohonan Barang sudah diproses')", True)
                End Try


            End If
        Else
            ClientScript.RegisterStartupScript(Me.[GetType](), "alert", "alert('Anda membatalkan pilihan')", False)
        End If
    End Sub
    ...
    Protected Sub btnProsesTT_Click(sender As Object, e As EventArgs)

    End Sub
    Protected Sub btnBatalkan_Click(sender As Object, e As EventArgs)
        Dim vpnButton As Button = CType(sender, Button)
        Dim Container As GridViewDataItemTemplateContainer = DirectCast(vpnButton.NamingContainer, GridViewDataItemTemplateContainer)
        Dim xPK As String = CStr(Container.Grid.GetRowValues(Container.VisibleIndex, Container.Grid.KeyFieldName).ToString)
        Dim xCEKORDER As Integer = CStr(Container.Grid.GetRowValues(Container.VisibleIndex, "CEKORDER").ToString)
        ...
        If confirmValue = "Yes" Then
            ...
            Try
                'xFungsi.UpdateData("TBL_ORDER", "STATUS=1", "KDORDER='" & xPK & "'")
                xFungsi.UpdateData("TBL_ORDER", "TGL_PROSES2=NULL,NAMAPROSES2=NULL,STATUS=1", "KDORDER='" & xPK & "'")
                ClientScript.RegisterStartupScript(Me.[GetType](), "alert", "alert('Pembatalan sudah diproses')", True)
                'xFungsi.UpdateData("TBL_ORDER", "", "KDORDER='" & xPK & "'")
            Catch ex As Exception
        ...
        End If
    End Sub
    Protected Sub gridDetail_RowUpdating(sender As Object, e As Data.ASPxDataUpdatingEventArgs)
        grid.DataBind()
    End Sub
    ...
    Protected Sub btnDownload_Click(sender As Object, e As EventArgs)
        ...
        Dim directoryPath As String = Server.MapPath(String.Format("~/{0}/", "Files/ORDER/" & xTAHUN))
